...
def remove_stop_words(text: str) -> str:
    ...
    text = text.split(" ")
    text = [word for word in text if not is_stop_word(word)]
    text = " ".join(text)
    return text
...
def process_document(doc_id: int, text: str = None, file_name: str = None):
    ...
    index_for_file = generate_index_for_an_article(doc_processed)
    add_to_index(index_for_file, doc_id)


def generate_index_for_an_article(text: str) -> Dict[str, List[int]]:
...
def add_to_index(index_for_file: Dict[str, List[int]], doc_id: int):
    """
    add the index to the global index
    """

    try:
        with open("indexPickle", "rb") as index_pickle:
    ...
    index_pickle = open(f"indexPickleAfterDoc{doc_id}", "w")
    temp_file = open(f"indexPickleAfterDoc{doc_id}", "w")
